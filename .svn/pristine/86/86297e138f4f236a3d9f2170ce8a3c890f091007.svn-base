    ...
    $scope.$on('$stateChangeStart', function (event, toState, toParams, fromState, fromParams) {
        if ((fromState.name === 'app.dashboard' && toState.name === 'member.login.v3') || fromState.name === 'app.dashboard' && toState.name === 'member.login.v1') {
            if ($rootScope.setting.login.remember) {
                toastr.warning("Already Logged-in", $rootScope.$localStorage.user.user_name, {timeOut: 1500});
            } else {
                toastr.warning("Already Logged-in", $rootScope.$sessionStorage.user.user_name, {timeOut: 1500});
            }
...
app.controller('headerController', function ($scope, $rootScope, $state) {
    ...
    $rootScope.$timeout(function () {
        if ($rootScope.setting.login.remember) {
            if (typeof($rootScope.$localStorage.user) !== 'undefined') {
                $rootScope.username = $rootScope.$localStorage.user.user_name;
            }
        } else {
            if (typeof($rootScope.$sessionStorage.user) !== 'undefined') {
                $rootScope.username = $rootScope.$sessionStorage.user.user_name;
            }
        }
    }, 10);
    ...
    if (typeof(localStorage.getItem('userimage_' + Cookies.get('loginname'))) !== 'string') {

    } else {
        $('body').find('.img-circle.circle-border').addClass('w-128');
        $rootScope.$timeout(function () {
            $('body').find('.navbar-user img,.nav-profile img,.user_image_profile').attr('src', localStorage.getItem('userimage_' + Cookies.get('loginname')));
        }, 0);
    }
});
...
app.controller('loginV1Controller', function ($scope, $rootScope) {
    ...
    if (typeof(localStorage.getItem('userimage_' + Cookies.get('loginname'))) !== 'string') {
        ...
        $('body').find('.img-circle.circle-border').addClass('w-128');
        $rootScope.$timeout(function () {
            $('body').find('.img-circle.circle-border').attr('src', localStorage.getItem('userimage_' + Cookies.get('loginname')));
        }, 100);
    }

    $scope.loginagain = function (form) {
        form = {username: $scope.loginname, password: form.password, remember: 'NO'}
        $rootScope.service.login(form);
    };
